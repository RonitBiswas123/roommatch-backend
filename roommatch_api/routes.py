...
def register(user: UserRegister):
    ...
    try:
        ...
        return {
            ...
            "access_token": token,
            "token_type": "bearer",
            "user": {"id": new_user[0], "name": new_user[1], "email": new_user[2], "branch": new_user[3], "year": new_user[4], "gender": new_user[5]}
        }
...
def login(credentials: UserLogin):
    conn, cur = get_cursor()
    try:
        cur.execute("SELECT id, name, email, password, branch, year, gender FROM users WHERE email = %s", (credentials.email,))
        user = cur.fetchone()
        ...
        return {
            ...
            "access_token": token,
            "token_type": "bearer",
            "user": {"id": user[0], "name": user[1], "email": user[2], "branch": user[4], "year": user[5], "gender": user[6]}
        }
    except HTTPException:
        raise
    except Exception as e:
...
def get_me(current_user = Depends(get_current_user)):
    ...
    try:
        ...
        return {"id": row[0], "name": row[1], "email": row[2], "branch": row[3], "year": row[4], "gender": row[5], "sleep_time": row[6], "wake_time": row[7], "study_hours": row[8], "cleanliness": row[9], "noise": row[10], "guests": row[11], "about": row[12]}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def get_users():
    ...
    try:
        ...
        rows = cur.fetchall()
        return {"users": [{"id": r[0], "name": r[1], "email": r[2], "branch": r[3], "year": r[4], "gender": r[5]} for r in rows], "total": len(rows)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        cur.close()
...
def get_user(user_id: int):
    ...
    try:
        cur.execute("""
            SELECT u.id, u.name, u.email, u.branch, u.year, u.gender,
                   p.sleep_time, p.wake_time, p.study_hours, p.cleanliness, p.noise, p.guests, p.about
...
def create_profile(user_id: int, profile: ProfileCreate):
    ...
    try:
        ...
        if existing:
            cur.execute("""
                UPDATE profiles SET sleep_time=%s, wake_time=%s, study_hours=%s,
                cleanliness=%s, noise=%s, guests=%s, about=%s, updated_at=CURRENT_TIMESTAMP
                WHERE user_id=%s
            """, (profile.sleep_time, profile.wake_time, profile.study_hours, profile.cleanliness, profile.noise, profile.guests, profile.about, user_id))
        else:
            cur.execute("""
                INSERT INTO profiles (user_id, sleep_time, wake_time, study_hours, cleanliness, noise, guests, about)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s)
            """, (user_id, profile.sleep_time, profile.wake_time, profile.study_hours, profile.cleanliness, profile.noise, profile.guests, profile.about))
        conn.commit()
...
def get_profile(user_id: int):
    ...
    try:
        cur.execute("""
            SELECT u.id, u.name, u.email, u.branch, u.year, u.gender,
                   p.sleep_time, p.wake_time, p.study_hours, p.cleanliness, p.noise, p.guests, p.about, p.updated_at
            FROM users u LEFT JOIN profiles p ON u.id = p.user_id WHERE u.id = %s
        """, (user_id,))
        row = cur.fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="User not found")
        return {"id": row[0], "name": row[1], "email": row[2], "branch": row[3], "year": row[4], "gender": row[5], "sleep_time": row[6], "wake_time": row[7], "study_hours": row[8], "cleanliness": row[9], "noise": row[10], "guests": row[11], "about": row[12], "updated_at": str(row[13]) if row[13] else None}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        query = """
            SELECT u.id, u.name, u.branch, u.year, u.gender,
                   p.sleep_time, p.wake_time, p.study_hours, p.cleanliness, p.noise, p.guests, p.about
            FROM users u JOIN profiles p ON u.id = p.user_id WHERE 1=1
        """
        params = []
        if branch:
        ...
        rows = cur.fetchall()
        return {"students": [{"id": r[0], "name": r[1], "branch": r[2], "year": r[3], "gender": r[4], "sleep_time": r[5], "wake_time": r[6], "study_hours": r[7], "cleanliness": r[8], "noise": r[9], "guests": r[10], "about": r[11]} for r in rows], "total": len(rows)}
    except Exception as e:
        print(f"ERROR: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
    ...
    try:
        ...
        cur.execute("""
            SELECT u.branch, u.year, u.gender,
                   p.sleep_time, p.wake_time, p.study_hours, p.cleanliness, p.noise, p.guests
        ...
        for s in students:
            ...
            if score >= min_score:
                results.append({
                    ...
                    "cleanliness": s[8], "noise": s[9], "guests": s[10], "about": s[11],
                    "compatibility": score,
